package org.defix.weather.service;
...
public class LocationDecoder {
    ...
    public static Location decode(String name) throws IOException, InterruptedException {
        ...
        logger.debug(STR."Current location response: \{res.body()}");
        ObjectMapper mapper = new ObjectMapper();
        try {
            JsonNode root = mapper.readTree(res.body()).get(0);
            logger.debug(STR."Current location: \{root.get("lat").asDouble()}:\{root.get("lon").asDouble()}");
